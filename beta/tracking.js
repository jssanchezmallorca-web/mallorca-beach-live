(function(){
  ...
  async function init(){if(state.ready||!configured())return state.ready;const key=joinKey();if(!key||!approved())return false;try{state.app=firebase.apps.find(a=>a.name==='tracker')||firebase.initializeApp(cfg.firebaseConfig,'tracker');state.auth=state.app.auth();state.db=state.app.database();await state.auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL);if(!state.auth.currentUser)await state.auth.signInAnonymously();state.uid=state.auth.currentUser.uid;await detectDeviceLabel();state.ready=true;const ref=state.db.ref(`locations/${state.uid}`);ref.onDisconnect().update({online:false,lastDisconnect:firebase.database.ServerValue.TIMESTAMP});return true}catch(e){console.warn('BEACH CAM tracking init error',e);return false}}
